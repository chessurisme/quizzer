export class ImportController {
	...
	async importQuiz() {
		...
		if (Array.isArray(this.selectedFiles) && this.selectedFiles.length > 1) {
			...
			try {
				const folderController =
					(this.quizController || window.quizController).folderController || window.folderController;
		...
		const isFullQuizExport =
			data && typeof data === "object" && Array.isArray(data.questions) && typeof data.type === "string";
		if (isFullQuizExport) {
			...
			quiz.questions = (Array.isArray(data.questions) ? data.questions : []).map((q) =>
				this.normalizeQuestion(q, type),
			);
